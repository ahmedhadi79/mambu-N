...
def mambu_fetch_latest_catalog(mambudb_streams, get_from_remote, working_dir):
    ...
    """
    if get_from_remote:
        catalog_command = "venv-tap-mambu/bin/tap-mambu --config {0}/tap_config.json --discover".format(
            working_dir
        ).split(
            " "
        )
...
def mambu_fetch(
    ...
    main_command = "venv-tap-mambu/bin/tap-mambu --config {0}/tap_config.json --catalog {0}/catalog.json --state {0}/state.json".format(
        working_dir
    ).split(
        " "
    )
    pipe_command = (
        "venv-target-jsonl/bin/target-jsonl --config {0}/tap_config.json".format(
            working_dir
        ).split(" ")
    )
...
# python deposit_transactions_wise_custom_fields.py --profile bb2-alpha-admin
